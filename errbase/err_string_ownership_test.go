...
func genEncoded(newVersion bool, ownsErrorString bool) errorspb.EncodedError {
	return errorspb.EncodedError{
		Error: &errorspb.EncodedError_Wrapper{
			Wrapper: &errorspb.EncodedWrapper{
				...
				EncoderKnowsAboutErrorStringOwnership: newVersion,
...
func TestDecodeOldVersion(t *testing.T) {
	...
	errOldEncoded := genEncoded(false, false)
	...
	// We ignore `WrapperOwnsErrorString` when knowledge flag is missing.
	errNewEncodedFlagMissing := genEncoded(false, true)
	errNewDecodedFlagMissing := errbase.DecodeError(context.Background(), errNewEncodedFlagMissing)
	tt.CheckEqual(errNewDecodedFlagMissing.Error(), "wrapper-error-msg: leaf-error-msg: extra info: leaf-error-msg")

	// Check to ensure that when both flags are present, we interpret things correctly.
	errNewEncoded := genEncoded(true, true)
	...
	errNew := errorspb.EncodedError{
		Error: &errorspb.EncodedError_Wrapper{
			Wrapper: &errorspb.EncodedWrapper{
				...
				EncoderKnowsAboutErrorStringOwnership: true,
