...
func encodeWrapper(ctx context.Context, err, cause error) EncodedError {
	...
	// This version is aware of the error string ownership feature.
	// It can only be overridden to `false` by an `opaqueWrapper`
	// that was originally encoded by an older version.
	knowsOwnError := true
	...
	if e, ok := err.(*opaqueWrapper); ok {
		...
		msg = e.prefix
		details = e.details
		knowsOwnError = e.encoderKnowsAboutErrorStringOwnership
	...
	return EncodedError{
		Error: &errorspb.EncodedError_Wrapper{
			Wrapper: &errorspb.EncodedWrapper{
				...
				MessagePrefix:                         msg,
				Details:                               details,
				EncoderKnowsAboutErrorStringOwnership: knowsOwnError,
